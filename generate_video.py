...
if __name__ == "__main__":
    ...
    def generate_once(img, p_text, p_guidance, p_steps, temperature):
        for idx in range(args.batch_size):
            ...
            kwargs = {
                ...
                "num_inference_steps": int(p_steps),
                "guidance_scale": float(p_guidance),
                "shift": args.shift,
            ...
            if local_rank == 0:
                ...
                video_out_file = (
                    f"{current_time}_liroai_cfg{p_guidance}_steps{p_steps}_temperature{temperature}_{p_text[:100].replace('/', '')}_{idx}.mp4"
                )
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if isinstance(data, dict) and "image_url" in data and "prompt" in data and "guidance" in data and "steps" in data:
                    return data["image_url"], data["prompt"], data["guidance"], data["steps"], data["temperature"]
        except Exception as e:
            print(f"API request failed: {e}")
        return None, None, None, None, None

    while True:
        task_image_url, task_prompt, p_guidance, p_steps, temperature = fetch_task()
        ...
        generate_once(img, prompt_txt, p_guidance, p_steps, temperature)
